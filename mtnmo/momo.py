...
class PayClass():
    ...
    def momotoken():
        ...
        payload = {}
        headers = {'Ocp-Apim-Subscription-Key': PayClass.collections_subkey,
    ...
    def momopay(amount, currency, txt_ref, phone_number, payermessage):
        ...
        payload = json.dumps({"amount": amount, "currency": currency, "externalId": txt_ref, "payer": {
                             "partyIdType": "MSISDN", "partyId": phone_number}, "payerMessage": payermessage, "payeeNote": payermessage})
        headers = {'X-Reference-Id': uuidgen, 'X-Target-Environment': PayClass.environment_mode, 'Ocp-Apim-Subscription-Key': PayClass.collections_subkey,
                   'Content-Type': 'application/json', 'Authorization': "Bearer "+str(PayClass.momotoken()["access_token"])}
        response = requests.request("POST", url, headers=headers, data=payload)
    ...
    def verifymomo(txn):
        ...
        payload = {}
        headers = {'Ocp-Apim-Subscription-Key': PayClass.collections_subkey, 'Authorization': "Bearer " +
                   str(PayClass.momotoken()["access_token"]), 'X-Target-Environment': PayClass.environment_mode}
    ...
    url = ""+str(accurl)+"/v1_0/apiuser"
    payload = json.dumps({"providerCallbackHost": "URL of host ie google.com"})
    headers = {'X-Reference-Id': disbursements_apiuser, 'Content-Type': 'application/json',
    ...
    def momotokendisbursement():
        ...
        payload = {}
        headers = {'Ocp-Apim-Subscription-Key': PayClass.disbursements_subkey,
    ...
    def momobalancedisbursement():
        ...
        payload = {}
        headers = {'Ocp-Apim-Subscription-Key': PayClass.disbursements_subkey, 'Authorization': "Bearer " +
                   str(PayClass.momotokendisbursement()["access_token"]), 'X-Target-Environment': PayClass.environment_mode}
        response = requests.request("GET", url, headers=headers, data=payload)
        json_respon = response.json()
        return json_respon
